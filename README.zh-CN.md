...
- 如果 Python 或 Node.js 安装失败，会转到下载安装包的重试流程
- 这个下载安装包的重试流程会先试官方地址，失败后再试预设国内镜像
- OpenClaw 先走默认 npm 源
- 如果 OpenClaw 安装失败，会自动切到预设的国内 npm 镜像再重试一次

所以它现在已经更接近“可以直接照着仓库在本地抄作业”的启动入口了，但还不算完全零依赖的一键安装器。`setup_local_console.bat` 的作用，就是先把这台机器当前到底缺什么、下一步该补什么讲清楚。
